...
@pytest.fixture
def default_retry_handler(mock_logger, mock_emitter):
...
class TestRetryHandlerSuccessfulOperations:
    ...
    @pytest.mark.asyncio
    async def test_success_on_first_attempt(self, default_retry_handler):
    ...
    @pytest.mark.asyncio
    async def test_success_after_retries(self, default_retry_handler):
    ...
    @pytest.mark.asyncio
    async def test_returns_operation_result(self, default_retry_handler):
...
class TestRetryHandlerPermanentErrors:
    ...
    @pytest.mark.asyncio
    async def test_no_retry_on_permanent_error(self, default_retry_handler):
    ...
    @pytest.mark.asyncio
    async def test_permanent_error_logged(self, mock_logger, default_retry_handler):
...
class TestRetryHandlerTransientErrors:
    ...
    @pytest.mark.asyncio
    async def test_retries_transient_errors(self, default_retry_handler):
    ...
    @pytest.mark.asyncio
    async def test_respects_max_retries_override(self, default_retry_handler):
        ...
        with pytest.raises(asyncio.TimeoutError):
            await default_retry_handler.execute_with_retry(
                operation, url="http://example.com/file.txt", max_retries=1
            )
    ...
    @pytest.mark.asyncio
    async def test_waits_between_retries(self, default_retry_handler, mocker):
...
class TestRetryHandlerEventEmission:
    ...
    @pytest.mark.asyncio
    async def test_emits_retry_event(self, mock_logger, mock_emitter):
        ...
        await handler.execute_with_retry(operation, url="http://example.com/file.txt")
    ...
    async def test_no_events_on_first_success(
        self, mock_emitter, default_retry_handler
    ):
...
class TestRetryHandlerLogging:
    ...
    @pytest.mark.asyncio
    async def test_logs_retry_attempts(self, mock_logger, mock_emitter):
    ...
    @pytest.mark.asyncio
    async def test_logs_final_failure(self, mock_logger, mock_emitter):
...
class TestRetryHandlerCategoriserIntegration:
    ...
    @pytest.mark.asyncio
    async def test_uses_injected_categoriser(self, mock_logger, mock_emitter):
